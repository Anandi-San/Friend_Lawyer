...
export const getKonsulForm = async (req, res) => {
    try {
        const response = await FormConsultant.findAll({
            attributes: ['uuid', 'full_name', 'email', 'phonenumber', 'day', 'hours', 'problem', 'clientid', 'lawyerid' ],
            include: [{
              model: Users,
              as: 'client',
              attributes: ['name']
    ...
    export const getKonsulFormById = async (req, res) => {
      ...
      export const createKonsulForm = async (req, res) => {
        try {
          const { full_name, email, phonenumber, day, hours, problem, clientid, lawyerid } = req.body;
      
          // Create a new record in the FormConsultant table
      ...
      export const updateKonsulForm = async (req , res) => {
        try {
            ...
            if(!formconsultant) return res.status(404).json({msg: "Data tidak ditemukan"});
            const {full_name, email, phonenumber, day, hours, problem, clientid, lawyerid} = req.body;
            if(req.role === "admin"){
                await Discussion.update({full_name, email, phonenumber, day, hours, problem, clientid, lawyerid},{
                    where:{
                        id: formconsultant.id
                    }
                });
            }else{
                if(req.userId !== formconsultant.userId) return res.status(403).json({msg: "Akses terlarang"});
                await FormConsultant.update({full_name, email, phonenumber, day, hours, problem, clientid, lawyerid},{
                    where:{
